...
TCPServer::~TCPServer() { close(_server_fd); }
...
int TCPAgent::send(const void* data, size_t size) {
  return this->send((char*)data, size);
}
...
int TCPAgent::getFd() { return _conn_fd; }
